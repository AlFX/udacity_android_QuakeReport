...
public final class QueryUtils {
    ...
    private static List<Earthquake> extractFeatureFromJson(String earthquakeJSON) {
        ...
        try {
            ...
            /* Create JSONObject from JSON response string */
            JSONObject baseJsonResponse = new JSONObject(earthquakeJSON);

            /*extract JSONArray associated with the key "features" which represents
            * a list of features (of the earthquakes) */
            JSONArray earthquakeArray = baseJsonResponse.getJSONArray("features");

            /* loop through each earthquake in earthquakeArray and create an
            Earthquake object */
            for (int i = 0; i < earthquakeArray.length(); i++) {

                /* get earthquake i-th object */
                JSONObject currentEarthquake = earthquakeArray.getJSONObject(i);

                /* for a given earthquake, extract the JSONObject associated with the key
                "properties" which represents a list of all the properties for that earthquake*/
                ...
                /* create a new earthquake object from JSONResponse input */
                Earthquake earthquake = new Earthquake(magnitude, place, time, url);

                /* add the new Earthquake object to the list of earthquakes */
            ...
            catch the exception here, so the app doesn't crash. Then print a log message
            with the message from the exception.*/
            Log.e("QueryUtils", "Problem parsing the earthquake JSON results", e);
        }

        // Return the earthquakes ArrayList to the caller (Earthquake Activity)
        return earthquakes;
    }
    ...
    private static String makeHttpRequest(URL url) throws IOException {
        ...
        try {
            ...
            if (inputStream != null) {
                ...
                makeHttpRequest(URL url) method signature specifies that an IOException could be thrown */
                inputStream.close();
            }
        }
        return jsonResponse;
